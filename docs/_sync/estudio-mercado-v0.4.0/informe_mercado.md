...
| Hay que decidir el IVA de importación / IEDMT | **NO incluir en las tablas.** SOLO mencionar el IEDMT en la sección "💶 Desglose de los 1.500 €" con el orden de magnitud realista (+2.200 a +3.500 € todo incluido). NO calcular por coche. **IVA: no se suma nunca** en una compra intracomunitaria (salvo regla 6/6000). |
...
¹ **Puesto en Huelva = precio Alemania + 1.500 € de gastos fijos** (1.129 € reales de `costes.md` + ~371 € de colchón). **Sin IEDMT**, que se calcula por coche según CO₂ y año. Un usado de la UE NO paga IVA en España.
...
>
> **Columna "Puesto en Huelva"**: precio alemán + 1.500 € de gastos fijos estimados (1.129 € reales de `costes.md` + ~371 € de colchón). **Sin IEDMT** (depende del CO₂ y año, se calcula cuando hay unidad concreta). Un usado de la UE NO paga IVA en España.
>
...
> Traer el GTI de Alemania te sale por **17.499 € puesto en Huelva** (sin IEDMT). El mismo coche en España cuesta **19.690 €** → te ahorras **2.191 €**.
...
>
> Los precios "puesto en Huelva" incluyen 1.500 € de gastos fijos estimados (1.129 € reales de `costes.md` + ~371 € de colchón). **Sin IEDMT** (se calcula para cada coche según CO₂ y año). Un usado de la UE NO paga IVA en España.
...
Precios "puesto en Huelva" (precio Alemania + 1.500 € de gastos, sin IEDMT):
...
> **Por qué una cifra redonda de 1.500 € y no el desglose técnico:** el estudio de mercado mira precios de compra en Alemania vs España. El IEDMT es **variable según el coche concreto** (CO₂, año), así que para decidir si **merece la pena importar** un modelo basta una cifra redonda que deje colchón.
>
> Los costes fijos reales documentados en `importacion-vehiculos/04-negocio/costes.md` suman **1.129 €**; los 1.500 € del informe son esos 1.129 € **más ~370 € de colchón** para imprevistos. No son un desglose distinto — es la misma fuente con margen.
>
> Cuando elijas una unidad concreta (Flujo A) se calcula el **IEDMT** exacto sobre la ficha técnica. **IVA no: ver el aviso de abajo.**

| Concepto | Estimado | Notas |
|---|---:|---|
| Transporte Alemania → Huelva | 900 € | Tarifa estándar Alemania-Huelva (`costes.md`) |
| Ausfuhrkennzeichen + seguro | 114 € | Matrícula temporal de exportación + seguro 15 días |
| ITV importación + tasas DGT | 115 € | Tarifa estándar, sube si hay reformas |
| *Subtotal real documentado* | *1.129 €* | Fuente única: `04-negocio/costes.md` |
| Colchón para imprevistos | ≈371 € | Redondeo al alza pedido por el usuario (23-ago-2026) |
| **TOTAL usado en el informe** | **1.500 €** | Sin IEDMT (variable por coche) |

**⚠️ IVA: un usado comprado en Alemania NO vuelve a tributar IVA en España.**
Esto se corrigió el 12-sep-2026 (mismo fallo detectado y eliminado en
`PrecioClienteCalculator.php` del panel, donde inflaba el precio ~6.700 € en un
coche de 32.000 €). No existe "valor en aduana" en una compra intracomunitaria.
Según `04-negocio/costes.md` §IVA:
- **Particular sin NIF-IVA (el caso normal de nuestros clientes):** paga el precio
  alemán con el IVA alemán ya dentro. **NO paga IVA español.** No sumar 21 % nunca.
- **Empresa con NIF-IVA intracomunitario:** paga el precio sin IVA alemán y liquida
  el 21 % español (modelo 309), que luego deduce.
- **Única excepción (regla 6/6000):** si el coche tiene **menos de 6 meses O menos de
  6.000 km**, sí se liquida el 21 % español aunque sea usado. Casi ningún coche del
  estudio entra aquí, pero si un modelo se mide con unidades casi nuevas hay que
  avisarlo explícitamente.
...
- Coche barato traerlo (sin IEDMT): +1.500 €
- Coche "traído y matriculado" todo incluido: **+2.200 € a +3.500 €** según el IEDMT
  del modelo (antes esta línea decía +3.500 a +5.500 € porque incluía un 21 % de IVA
  que no se paga).
