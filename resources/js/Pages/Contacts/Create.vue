...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ t('contacts.add') }}</h2>
        </template>

        <div class="py-8">
            <div class="mx-auto max-w-2xl space-y-6 px-4 sm:px-6 lg:px-8">
                <PageHeader :title="t('contacts.add')" :subtitle="t('contacts.add_subtitle')">
                    <template #actions>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <FormSection :title="t('contacts.tags_notes')">
                        <FormField :label="t('contacts.field_tags')" :hint="t('contacts.tags_hint')">
                    ...
                    <div class="flex items-center justify-end gap-3 rounded-2xl bg-gray-50 px-6 py-4 ring-1 ring-gray-200">
                        ...
                        <button type="submit" :disabled="form.processing" class="inline-flex items-center gap-2 rounded-lg bg-estoril-600 px-5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-estoril-500 disabled:opacity-50">
                            ...
                            {{ form.processing ? t('clients.saving') : t('contacts.create_contact') }}
                        </button>
                    </div>
                </form>
            </div>
