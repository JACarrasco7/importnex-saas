...
import { Head, Link, useRouter } from '@inertiajs/vue3';
import {
    CheckBadgeIcon,
    ClockIcon,
    ScaleIcon,
...
// Navega con los filtros actuales preservados
...
function kpiMeta(kpi) {
    // KPI correcto (verde) o por debajo del objetivo (ámbar/rojo)
...
const kpiCards = computed(() => [
    { key: 'precision_veredictos', label: 'Precisión de veredictos', icon: CheckBadgeIcon, suffix: '%', ...props.kpis.precision_veredictos },
    { key: 'tiempo_hasta_venta', label: 'Tiempo hasta venta', icon: ClockIcon, suffix: 'd', ...props.kpis.tiempo_hasta_venta },
    { key: 'desviacion_precio', label: 'Desviación de precio', icon: ScaleIcon, suffix: '%', ...props.kpis.desviacion_precio },
    { key: 'tasa_falsos_positivos', label: 'Tasa falsos positivos', icon: ExclamationTriangleIcon, suffix: '%', ...props.kpis.tasa_falsos_positivos },
]);

const tendenciaMax = computed(() => Math.max(1, ...(props.tendencia || []).map((t) => Number(t.precision) || 0)));

const estadoVenta = (c) => {
    if (c.estado === 'vendido') return { label: 'Vendido', cls: 'bg-emerald-50 text-emerald-700 ring-emerald-200' };
    if (c.estado === 'no_vendido') return { label: 'No vendido', cls: 'bg-rose-50 text-rose-700 ring-rose-200' };
    return { label: 'Pendiente', cls: 'bg-amber-50 text-amber-700 ring-amber-200' };
};
</script>

<template>
    <Head title="KPIs · Importación" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">KPIs · Importación de vehículos</h2>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <PageHeader
                        title="Rendimiento del skill de importación"
                        subtitle="KPIs de precisión de veredictos y ventas por periodo"
                    />

                    <div class="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm ring-1 ring-gray-200">
                        <Link :href="buildHref({ periodo: prevPeriodo })" class="rounded-lg p-1 text-gray-500 hover:bg-gray-100 hover:text-gray-900" aria-label="Mes anterior">
                ...
                <div class="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-200 sm:flex-row sm:items-end">
                    <div class="flex-1">
                        <label class="mb-1 block text-xs font-medium text-gray-700">Marca</label>
                        <select
                            :value="filtros?.brand || ''"
                    ...
                    <div class="flex-1">
                        <label class="mb-1 block text-xs font-medium text-gray-700">Plataforma / País</label>
                        <select
                            :value="filtros?.pais || ''"
                    ...
                    <div v-for="kpi in kpiCards" :key="kpi.key" class="overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                        ...
                        <div v-if="kpi.valor !== null" class="mt-3 flex items-center gap-1.5">
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1" :class="kpiMeta(kpi).color + ' ' + kpiMeta(kpi).text + ' ' + kpiMeta(kpi).ring">
                                Objetivo: {{ kpi.objetivo }}{{ kpi.suffix }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
                    <div class="border-b border-gray-200 px-6 py-4">
                        <h3 class="text-base font-semibold text-gray-900">Tendencia de precisión (6 meses)</h3>
                    </div>
                    <div v-if="tendencia.length" class="p-6">
                        <div class="flex items-end justify-between gap-2">
                            <div v-for="t in tendencia" :key="t.periodo" class="flex flex-1 flex-col items-center gap-2">
                                <div class="flex w-full flex-col items-center justify-end" style="height: 160px">
                                    ...
                                    <div
                                        ...
                                        :style="{ height: (t.precision !== null ? (t.precision / tendenciaMax) * 140 : 4) + 'px', backgroundColor: t.precision === null ? '#e5e7eb' : (t.precision >= 80 ? '#10b981' : t.precision >= 60 ? '#f59e0b' : '#f43f5e') }"
                                        :title="`${t.periodo}: ${t.precision !== null ? t.precision + '%' : 'sin datos'} (${t.volumen} cierres)`"
                                    ></div>
                                </div>
                                <span class="text-xs text-gray-500">{{ t.periodo }}</span>
                                <span class="text-[10px] text-gray-400">{{ t.volumen }} cierres</span>
                            </div>
                        </div>
                    </div>
                    <div v-else class="p-10 text-center text-sm text-gray-500">Sin datos de tendencia.</div>
                </div>

                <!-- Tabla de cierres -->
                <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
                    <div class="border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">Cierres del periodo</h3>
                        <span class="text-sm text-gray-500">{{ cierres.length }} registros</span>
                    </div>
                    ...
                    <div v-if="cierres.length" class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Coche</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Marca / Modelo</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Plataforma</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Fecha investigación</th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Veredicto</th>
                                    <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Precio objetivo</th>
                                    <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Precio final</th>
                                    <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Desviación</th>
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="c in cierres" :key="c.id" class="hover:bg-gray-50">
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        ...
                                        <span v-else class="text-gray-500">{{ c.coche_id }}</span>
                    ...
                    <div v-else class="p-10 text-center text-sm text-gray-500">
                        No hay cierres registrados en este periodo. Los cierres se registran al cerrar investigaciones del skill.
                    </div>
                </div>
            </div>
        </div>
