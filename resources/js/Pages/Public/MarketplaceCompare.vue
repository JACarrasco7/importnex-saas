        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 text-center">
                    <div class="mx-auto mb-4 inline-flex h-14 w-14 items-center justify-center rounded-full bg-estoril-100">
                ...
                <!-- Empty state -->
                <div v-if="cars.length === 0" class="rounded-2xl bg-white p-12 text-center shadow-sm ring-1 ring-gray-200">
                ...
                <!-- Compare table -->
                <div v-else class="overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="sticky left-0 z-10 bg-gray-50 px-4 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                                ...
                                <th v-for="car in cars" :key="car.id" class="px-4 py-4 text-center">
                                    <Link :href="route('marketplace.show', car.id)" class="block">
                                        ...
                                        <p class="mt-2 text-sm font-semibold text-gray-900">{{ car.brand }} {{ car.model }}</p>
                                    </Link>
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="row in rows" :key="row.key">
                                <td class="sticky left-0 z-10 bg-white px-4 py-3 text-sm font-medium text-gray-700">
                                    {{ row.label }}
                                </td>
                                <td v-for="car in cars" :key="car.id" class="px-4 py-3 text-center text-sm text-gray-900">
                                    {{ row.format(car) }}
                                </td>
                            </tr>
                            <!-- Action row -->
                            <tr class="bg-gray-50">
                                <td class="sticky left-0 z-10 bg-gray-50 px-4 py-4"></td>
