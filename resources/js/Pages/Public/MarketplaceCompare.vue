...
const rows = computed(() => [
    { key: 'price', label: t('marketplace.compare.price'), format: (c) => formatCurrency(c.purchase_price) },
    { key: 'year', label: t('marketplace.compare.year'), format: (c) => c.year },
    { key: 'mileage', label: t('marketplace.compare.mileage'), format: (c) => c.mileage ? `${c.mileage.toLocaleString('es-ES')} km` : '—' },
    { key: 'fuel', label: t('marketplace.compare.fuel'), format: (c) => c.fuel || '—' },
    { key: 'transmission', label: t('marketplace.compare.transmission'), format: (c) => c.transmission || '—' },
    { key: 'power', label: t('marketplace.compare.power'), format: (c) => c.power ? `${c.power} CV` : '—' },
    { key: 'doors', label: t('marketplace.compare.doors'), format: (c) => c.doors || '—' },
    { key: 'color', label: t('marketplace.compare.color'), format: (c) => c.color || '—' },
    { key: 'verdict', label: t('marketplace.compare.verdict'), format: (c) => c.verdict || '—' },
    { key: 'traffic_light', label: t('marketplace.compare.traffic_light'), format: (c) => c.traffic_light || '—' },
    { key: 'estimated_saving', label: t('marketplace.compare.savings'), format: (c) => c.estimated_saving ? formatCurrency(c.estimated_saving) : '—' },
]);
</script>

<template>
    <Head :title="t('marketplace.compare.title')" />
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 text-center">
                    ...
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl dark:text-white">
                        {{ t('marketplace.compare.title') }}
                    </h1>
                    <p class="mt-2 text-lg text-gray-600 dark:text-gray-400">
                        {{ t('marketplace.compare.subtitle') }}
                    </p>
                </div>
                        ...
                        <thead class="bg-gray-50 dark:bg-asphalt-800">
                            <tr>
                                <th class="sticky left-0 z-10 bg-gray-50 px-4 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:bg-asphalt-800 dark:text-gray-400">
                                    {{ t('marketplace.compare.features') }}
                                </th>
