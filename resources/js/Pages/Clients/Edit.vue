...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit: {{ client.name }}</h2>
        </template>

        <div class="py-8">
            <div class="mx-auto max-w-3xl space-y-6 px-4 sm:px-6 lg:px-8">
                <PageHeader :title="`Edit ${client.name}`" subtitle="Update client details">
                    <template #actions>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3 rounded-2xl bg-gray-50 px-6 py-4 ring-1 ring-gray-200">
                        <Link :href="route('clients.show', client.id)" class="text-sm font-semibold text-gray-700 hover:text-gray-900">Cancelar</Link>
