...
class ImportValuationApiController extends Controller
{
    ...
    /**
     * Adjunta un PDF briefing generado por el chat al expediente de un coche existente.
     *
     * Uso desde el chat (curl):
     *   curl -X POST https://jjimportmotors.on-forge.com/api/cars/{car_id}/briefing-pdf \
     *     -H "X-Import-Token: <token>" \
     *     -H "Content-Type: multipart/form-data" \
     *     -F "file=@briefing.pdf"
     *
     * §10.1 — implementado tras la auditoría (antes la ruta existía pero el método no).
     */
    public function attachBriefing(Request $request, Car $car): JsonResponse
    {
        $org = $request->attributes->get('import_org');

        // Auditoría 3 (#6) — el coche debe pertenecer a la org autenticada
        if ($car->organization_id !== $org->id) {
            return response()->json(['error' => 'Car not found in this organization.'], 404);
        }
        ...
        if (! $request->hasFile('file')) {
            return response()->json([
                ...
                'hint' => 'Send as multipart/form-data with field name "file".',
            ], 422);
        }
        ...
        if ($file->getClientOriginalExtension() !== 'pdf' || $file->getMimeType() !== 'application/pdf') {
            return response()->json(['error' => 'Only PDF files are accepted.'], 422);
        }
        ...
        try {
            ...
            $storagePath = "importnex/briefings/{$orgDirName}/vehicles/{$car->id}";
            $fileName = 'briefing_'.now()->format('Ymd-His').'.pdf';

            $path = $file->storeAs($storagePath, $fileName, 'local');

            Log::info('Briefing PDF adjuntado', [
                'car_id' => $car->id,
                'organization_id' => $org->id,
                'path' => $path,
            ]);
            ...
            Log::error('ImportValuationApiController::attachBriefing failed', ['error' => $e->getMessage()]);

            return response()->json(['error' => 'Attach failed: '.$e->getMessage()], 500);
        }
    }
